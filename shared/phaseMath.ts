...
export function judgePhaseAttempt(attemptTheta: number, target: PhaseTarget): PhaseJudgement {
  ...
  if (target.kind === 'theta') {
    ...
    let feedback: string;
    if (ok) {
      feedback = '목표와 같은 배치예요. 왜 이 위치에서 이런 모양이 보이는지 태양빛을 받는 부분과 지구에서 보이는 부분으로 설명해 볼까요?';
    } else if (fractionMatch && waxingMatch === false) {
      feedback = targetWaxing
        ? '밝게 보이는 비율은 비슷해요. 하지만 네 모형은 보름을 지나 기우는 위치예요. 목표 자료의 다음 날짜에는 밝은 부분이 늘어날까요, 줄어들까요?'
        : '밝게 보이는 비율은 비슷해요. 하지만 네 모형은 아직 보름 전, 차오르는 위치예요. 목표의 밝은 쪽이 왼쪽인지 오른쪽인지 다시 살펴보고 달을 반대편으로 옮겨 볼까요?';
    } else if (fraction < targetFraction - 0.12) {
      feedback = '지금 위치에서는 밝은 부분이 목표보다 작게 보여요. 달이 태양의 반대편에 가까워질수록 지구에서 보이는 밝은 부분은 어떻게 변할까요? 달을 태양에서 더 먼 쪽으로 옮겨 보세요.';
    } else if (fraction > targetFraction + 0.12) {
      feedback = '지금 위치에서는 밝은 부분이 목표보다 크게 보여요. 달이 태양과 같은 쪽에 가까워질수록 지구에서 보이는 밝은 부분은 어떻게 변할까요? 달을 태양 쪽으로 옮겨 보세요.';
    } else {
      feedback = '거의 가까워요. 밝은 면의 경계선 모양을 목표와 비교하면서 조금만 더 움직여 보세요.';
    }
  ...
  if (ok) {
    feedback =
      target.waxing === null
        ? '밝은 비율이 자료와 비슷한 범위예요. 사진의 회전이나 시각을 알 수 없어 차오름·기움은 판정하지 않았어요. 두 가능성 중 어느 쪽인지 자료의 날짜로 따져 볼까요?'
        : '자료와 비슷한 범위의 배치예요. 자료와 모형이 완전히 같지 않다면 어떤 이유가 있을지 생각해 보세요.';
  } else if (!fractionMatch) {
    feedback =
      fraction < target.minFraction
        ? '모형의 밝은 부분이 자료보다 작아요. 달을 태양에서 더 먼 쪽으로 옮기면 어떻게 될까요?'
        : '모형의 밝은 부분이 자료보다 커요. 달을 태양 쪽으로 옮기면 어떻게 될까요?';
  } else {
    feedback = '밝은 비율은 비슷하지만 차오름과 기움이 달라요. 자료의 날짜가 보름 앞인지 뒤인지 확인해 보세요.';
  }
  return {
    ok,
    fractionMatch,
